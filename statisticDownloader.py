...
def getKeys(dictionary,keys):
    '''Get the keys for crunchbase and google from the keys.txt file'''
    for dictKey in dictionary.keys():
        dictionary[dictKey] = [key.split(':')[1].strip(' ,\t\n\r') for key in keys 
            if key.split(':')[0].strip(' ,\t\n\r') == dictKey][0]
    return dictionary

def dataSummarizer(inputDict, 
									crunchBase, 
									googleTrends= None, 
									googleNews = None, 
									frequency = 'Q'):
...
def statisticDownloader(industry, startDate, endDate, frequency):
	  '''Takes the wished input arguments, calls the export functions and saves or return the complete dataframe'''
